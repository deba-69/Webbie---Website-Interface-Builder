...
if(isset($_POST["back"]))
{
    echo "<script>window.alert('Going back ko section page');</script>";
    echo "<script>window.open('addSection.php','_self')</script>";
}
else if(isset($_POST["submit"]))
{
    $cat_id = $_POST["category"]; 
    setcookie("sect",$cat_id);
    echo "<script>window.open('addProduct.php','_self');</script>";
    /*$query = "delete from section where sect_id = $cat_id and user_id in (select user_id from user_custom where email = '$email')";
